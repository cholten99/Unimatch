...
geoXML3.parser = function (options) {
  ...
  var parse = function (urls, complete_callback) {
    ...
    for (var i = 0; i < urls.length; i++) {
      ...
      geoXML3.fetchXML(thisDoc.url, function (responseXML) {render(responseXML, thisDoc, complete_callback);});
    }
  };
  ...
  var render = function (responseXML, doc, complete_callback) {
    ...
    if (!responseXML) {
      ...
      if (doc.internals.remaining === 0) {
        // Were done processing this set of KML documents
  ...
  var createMarker = function (placemark, doc) {
    ...
    google.maps.event.addListener(marker, 'click', function() {
        highlight_marker_from_obj(marker);
    });
  ...
  if (!xhrFetcher) {
    ...
    xhrFetcher.onreadystatechange = function () {
      if (xhrFetcher.readyState === 4) {
        ...
        // Were done with this fetcher object
